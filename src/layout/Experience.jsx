    ...
    <section className = "p-2">
      <div className={`border rounded-lg p-6 pb-[5.4rem] transition-colors duration-300 ${
        ...
        <div className="relative space-y-6">
          <div className={`absolute left-1.5 top-2 bottom-2 w-px ${darkMode ? 'bg-white/10' : 'bg-gray-300'}`}></div>
          ...
          {experiences.map((exp, index) => (
            <div key={index} className="relative pl-8 group cursor-default">
              <div className={`absolute left-0 top-1.5 w-3 h-3 rounded-full transition-all duration-300 z-10 hover:scale-120 ${
                exp.current 
                  ? (darkMode ? 'bg-white border-white' : 'bg-black border-black') + ' scale-110'
                  : (darkMode ? 'bg-black border-white/20' : 'bg-white border-gray-300') + ' border-2'
              }`}></div>
            ...
            : = past job */}
              ...
              <div className="flex flex-row items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  <h3 className={`text-sm font-normal leading-none ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                    {exp.title}
                  </h3>
                  <p className={`text-xs mt-1.5 font-extralight ${darkMode ? 'text-gray-200/60' : 'text-gray-500'}`}>
                    {exp.company}
                  </p>
                </div>
